...
import { adminAuthMiddleware, keyAuthMiddleware } from "./middleware/auth.js";
...
adminRouter.post("/keys", (req, res) => {
	...
	if (!KEY_NAME_REGEX.test(name)) {
		res.status(400).json({
			error:
				"name must be 1-50 alphanumeric characters, hyphens, or underscores",
		});
		return;
	}
...
function validateGenerateBody(body: Record<string, unknown>): {
	error?: string;
} {
...
function logRequest(
	...
	if (usage.cache_creation_input_tokens)
		console.log(`  Cache created: ${usage.cache_creation_input_tokens}`);
	if (usage.cache_read_input_tokens || usage.cached_input_tokens) {
		console.log(
			`  Cache read: ${usage.cache_read_input_tokens || usage.cached_input_tokens}`,
		);
	}
...
app.post("/generate", async (req, res) => {
	...
	try {
		...
		console.error(
			"[generate] Claude generation error:",
...
app.post("/generate-codex", async (req, res) => {
	...
	try {
		...
		console.error(
			"[generate-codex] Codex generation error:",
			err instanceof Error ? err.message : err,
		);
		res.status(500).json({ error: "Generation failed" });
	}
...
// Gemini CLI (supports stream: true)
app.post("/generate-gemini", async (req, res) => {
	...
	if (stream) {
		...
		const handle = streamWithGemini(
			{ systemPrompt, userPrompt, model },
			{
				defaultModel: config.geminiDefaultModel,
				timeoutMs: config.geminiTimeoutMs,
				maxBuffer: config.geminiMaxBuffer,
			},
			{
				onText: (chunk) => {
					sendSSEEvent(res, "text", { text: chunk });
				...
				onUsage: (usage, costUsd, durationMs) => {
					...
					const usedModel = model || config.geminiDefaultModel;
					keyManager.logRequest({
						keyName: req.keyName || "anonymous",
						provider: "gemini",
						model: usedModel,
				...
				onError: (err) => {
					console.error(
						"[generate-gemini] Gemini streaming error:",
						err.message,
					);
					sendSSEError(res, "Generation failed");
				},
	...
	try {
		...
		console.error(
			"[generate-gemini] Gemini generation error:",
			err instanceof Error ? err.message : err,
		);
		res.status(500).json({ error: "Generation failed" });
	}
...
app.listen(config.port, () => {
	...
	console.log(
		`  Keys: ${keyManager.hasKeys() ? `${keyManager.keyCount()} key(s)` : "NONE (auth disabled)"}`,
	);
	console.log(
		`  Admin: ${config.adminKey ? "enabled" : "DISABLED (no BRIDGE_ADMIN_KEY set)"}`,
	);
	console.log(
		`  Rate limit: ${config.rateLimitMaxRequests} req / ${config.rateLimitWindowMs / 1000}s`,
	);
