...
export const config = {
	...
	// Gemini CLI
	geminiDefaultModel: process.env.GEMINI_DEFAULT_MODEL || "auto-gemini-3",
	geminiTimeoutMs: requireInt("GEMINI_TIMEOUT_MS", "180000", 1000),
	geminiMaxBuffer: requireInt("GEMINI_MAX_BUFFER_BYTES", "10485760", 1024),
};

if (!config.adminKey) {
	console.warn(
		"[config] WARNING: BRIDGE_ADMIN_KEY is not set — admin endpoints will reject all requests",
	);
}
